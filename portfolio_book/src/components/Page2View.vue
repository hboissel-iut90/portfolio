  ...
  <v-container style="font-size: large;">
    ...
    >
      ...
      <v-container>
        <v-card-text class="txt">
          <p>
            La première étape de cette mission était de trouver des API capables de nous fournir des données pertinentes
            pour le laboratoire, en temps réel,
            de préférence gratuitement, et de préférence sans passer par la création d’un compte. J’ai alors débuté les
            recherches par l’API météo.
          </p>
        </v-card-text>
      </v-container>
      <v-container>
        <v-container
          ...
          <v-card-text class="dirCol align">
            <img class="img2 borderIuntitledmg" alt="Image not found" src="@/assets/MeteoFrance.png"/>
      ...
      <v-container>
        <v-card-text class="txt">
          ...
          <v-container class="txt" style="padding-inline: 60px">
            <ul>
              <li><h4>OpenMétéo, API gratuite sans compte et qui permet de faire cinq-cents requêtes par minutes et
                dix-mille par jour possédant un grand nombre de données en temps réel;</h4></li>
              <br>
              <li><h4>MétéoStat, API conseillée par mon tuteur, pouvant fournir un grand nombre de données
                gratuitement.</h4></li>
              <br>
            </ul>
          </v-container>
          <p>
            J’ai fait le choix de prendre OpenMétéo qui répondait à tous les critères, sachant que MétéoStat ne fournit
            que des tableurs et donc aucune donnée en temps réel.
          </p>
        </v-card-text>
        <v-card-text class="txt">
          <p>
      ...
      <v-container>
        <v-card-text class="txt">
          <p>
            Comme présenté sur la figure 1, j’ai pu trouvé six API de trafic routier présentant chacune leurs propres
            données.
          ...
          <p>
            Par rapport aux API météo, il est plus difficile d’accéder à des données de trafic routier gratuitement et
            en temps réel,
            pour première raison que chaque API demande d’avoir un compte,
            mais surtout la seconde qui provient du fait qu’il est difficile pour les gestionnaires de routes de fournir
            des données en continu
            car cette tâche peut être chronophage et peut ne pas être leur priorité, mais aussi que toutes les routes de
            France ne sont pas publiques,
      ...
      <v-container>
        <v-card-text class="txt">
          <p>
            Le code d’implémentation des données météorologiques et celui des données de trafic routier sont les mêmes à
            quelques détails près,
            je ne présenterai alors que le code pour l’API météo.
          </p>
          <p>
            Comme expliquer dans l’introduction de cette mission, l’implémentation s’est passé en 4 étapes :
          </p>
          <v-container class="txt" style="padding-inline: 60px">
            <ul>
              <li><h4>Envoi d’une requête HTTP à l’API souhaitée;</h4></li>
              <br>
              <li><h4>Transformation des données du format JSON au format GSON;</h4></li>
              <br>
              <li><h4>Triage des données voulues à partir du GSON;</h4></li>
              <br>
              <li><h4>Insertion dans le fichier de données.</h4></li>
              <br>
            </ul>
          </v-container>
          <p>
            Pour expliquer plus en détail, la réponse à la requête HTTP est au format JSON (= JavaScript Object
            Notation) pour OpenMétéo,
            format qui n’est pas compréhensible en Java, ce qui demande de passer par une dépendance appelé GSON (=
            Google JSON) qui permet de transformer le JSON en liste Java.
            Ensuite, on prend les données qui nous intéressent, c’est-à-dire supprimer des morceaux de la requête pour
            ne sélectionner qu’un bout,
            pour finalement insérer les données.
          </p>
        </v-card-text>
      </v-container>
      <v-container>
        <v-card-text class="txt">
          <p>
            Pour conclure, cette mission m’a permis d’améliorer mon savoir dans les différentes dépendances existantes
            en android et mon savoir-faire dans la recherche d’informations, comme la recherche d’API.
          </p>
        </v-card-text>
      </v-container>
      <v-container
